      ...
      {health.errors.length > 0 && (
        <div className="mt-6 bg-red-50 border-2 border-red-300 rounded-xl p-8">
          <div className="flex items-center gap-4 mb-6">
            <ExclamationCircleIcon className="w-12 h-12 text-red-600 shrink-0" />
            <p className="font-bold text-red-900 text-2xl">
              {health.errors.length === 1
                ? "1 issue found"
                : `${health.errors.length} issues found`}
            </p>
          </div>
          <ul className="space-y-4 ml-4">
            {health.errors.map((error, index) => (
              <li key={index} className="flex items-center gap-4">
                <span className="text-red-500 font-bold text-3xl shrink-0 leading-none">
                  ▸
                </span>
                <span className="text-base text-red-800 leading-snug">
                  {error}
                </span>
              </li>
            ))}
